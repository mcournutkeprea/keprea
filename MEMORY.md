...
| 03 juil. 2026 (3) | Stats "Agriculture en transition" : utilisateur confirme les chiffres 84%/100%/5 500 comme fiables et publiables → "XXX" restaurés en valeurs définitives dans `Problem.tsx` (commit `0b97b76`). Décision classée résolue. |
| 03 juil. 2026 (4) | Décisions Boosters tranchées : contraste NPK 15-15-15 vs Fertea432 4-3-2 validé tel quel (exemple comparatif volontaire conservé). Claim "azote rapide" confirmé à ne pas ajouter (contredirait la libération lente déjà annoncée) — n'existe pas dans le code, rien à modifier. Les deux décisions en attente de Boosters sont résolues. |
