...
| 02 juil. 2026 | Audit SEO complet (`/seo audit`, score 53/100, détails `keprea.vercel.app-audit/`). **Phase 1** : title/meta dupliqués supprimés, canonicals (14 pages), JSON-LD Organization sur keprea.vercel.app (à rebasculer à la migration), "biostimulant" ajouté à Boosters, Team confirmée par l'utilisateur. **Phase 2** : vidéo hero 13→1,9 Mo, PNG→WebP 68 Ko, BreadcrumbList (13 pages), `/contact` mobile resserré, OG/Twitter par page, headers sécurité (CSP différée), ~64 Mo vidéos orphelines + `ProductsSchema.tsx` mort supprimés, Footer aligné sur Navigation. **Phase 3** (contenu & autorité) : `src/lib/schema.ts` (Article/FAQPage/Service JSON-LD) ; `/pourquoi-le-biocontrole` 335→1055 mots avec stats sourcées (Ecophyto 2030, Alliance Biocontrôle, Code rural — vérifiées par recherche web) ; 4 pages produits 595-747→820-848 mots (FAQ étendues + FAQPage/Service schema) ; `/qui-sommes-nous` et `/notre-production` enrichis (222→553 et 145→512 mots, jalons/chiffres réels) ; stats internes reformulées "données internes Keprea" (dissociées des orgs tierces citées à tort) ; `/ressources` passé `noindex` (retiré sitemap) faute de contenu réel ; byline + date sur pages à claims réglementaires. **Phase 4** (monitoring, 02/07/2026, site toujours sur vercel.app) : `robots.txt` corrigé (pointait vers `keprea.com/sitemap.xml`, 404 permanent car domaine pas encore live) ; `public/llms.txt` créé. CWV réel et Search Console restent bloqués (pas de `GOOGLE_API_KEY`, pas de cutover) ; volontairement pas de sources INRAE/FAO ajoutées aux stats produits (essais internes Keprea, pas de données tierces à citer — risque de faire croire à une validation externe inexistante). Détails complets dans `ERRORS.md`. |
| 02 juil. 2026 (2) | "Made in France" remplace "Dole/Damparis/Jura" dans tout le contenu marketing visible (Hero, Testimonials, Production, Innovation, Pourquoi Biocontrôle, Qui Sommes-Nous, Notre Production) — demande utilisateur : visiteurs étrangers ne connaissent pas ces villes. **Conservé volontairement** : adresse réelle dans Footer/Contact/Mentions légales/Politique confidentialité/FranceMap (obligation légale + cohérence NAP SEO local), et tous les `<title>`/`<meta>` SEO (ciblage recherche locale française). |
